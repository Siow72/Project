...
def split_skill_text(skills_text):
    """Split comma/semicolon/newline-separated skills into clean items."""
    skills_text = safe_text(skills_text)
    ...
    for chunk in skills_text.replace(";", ",").replace("\n", ",").split(","):
        skill = chunk.strip()
        if skill:
...
def collect_resume_skills(skills, experiences):
    """Combine main skills and skills used in work experience without duplicates."""
    combined = []
    seen = set()

    def add_skill(skill):
        skill = safe_text(skill)
        if not skill:
            return
    ...
    for skill in split_skill_text(skills):
        add_skill(skill)

    for exp in experiences or []:
        for skill in split_skill_text(exp.get("company_skills", "")):
            add_skill(skill)

    return combined


def build_combined_skills_text(skills, experiences):
    """Return combined skills as comma-separated text for the Ollama prompt."""
    return ", ".join(collect_resume_skills(skills, experiences))


def build_skills_markdown(skills, experiences):
    """Build the Skills section before Ollama generation."""
    skill_items = collect_resume_skills(skills, experiences)
...
def convert_json_to_markdown(output, original_input=None):
    ...
    # ----------------------
    summary = safe_text(output.get("summary"))
    ...
    if isinstance(skills, list):
        for skill_group in skills:
            ...
            category = safe_text(skill_group.get("category"))
            ...
            if clean_items:
                if category:
                    skill_lines.append(f"{category}: {', '.join(clean_items)}")
                else:
                    skill_lines.append(", ".join(clean_items))

    if skill_lines:
        resume_md += "\n\n---\n\n## Skills\n\n"

        for skill_line in skill_lines:
            resume_md += skill_line + "\n"

    # ----------------------
    # Project Experience
    # Keep project name from original input.
    # Use bullet points generated by Ollama.
    # ----------------------
    ...
    if generated_projects or original_projects:
        ...
        for i in range(project_count):
            ...
            # Skip empty project
            if not project_name and not clean_bullets:
                continue
    ...
    # - Bullet points
    #
    # Ignore location, job_type, duration from Ollama.
    # ----------------------
    ...
    if generated_experiences or original_experiences:
        ...
        for i in range(experience_count):
            ...
            start_month = safe_text(original_exp.get("start_month", ""))
            start_year = safe_text(original_exp.get("start_year", ""))
            end_month = safe_text(original_exp.get("end_month", ""))
            end_year = safe_text(original_exp.get("end_year", ""))
            ...
            if not position and not company and not period and not clean_bullets:
                continue
...
def build_full_resume_text(
    ...
    skills_md = build_skills_markdown(skills, experiences)
    ...
    if project_list:
        ...
        for project in project_list:
            ...
            if project_name or project_description or project_achievement:
                ...
                project_md += f"{project_description or ''}\n\n"
                project_md += (
                    f"**Achievement:** "
                    f"{project_achievement or ''}\n\n"
                )
...
def render_resume_builder_page():
    ...
    with right:
        ...
        combined_skills_text = build_combined_skills_text(skills, experiences)
